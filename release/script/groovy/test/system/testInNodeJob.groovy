...
// 提交指令
//ssh.system("srun -N 1 -n 1 -o out -c 6 ./jse ${ioFiles.i('<self>')}", ioFiles); // 通过 key 来获取注册的文件名
ssh.system("echo -e '#!/bin/bash\\nsrun -n 1 -c 6 ./jse ${ioFiles.i('<self>')}' | sbatch -N 1 -p debug", ioFiles); // 通过 key 来获取注册的文件名
