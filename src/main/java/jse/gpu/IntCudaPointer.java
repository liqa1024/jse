 ...
 * <p>
 * 注意编译器不同可能的 {@code int} 大小也不同，因此这里需要针对 nvcc 的情况重新获取大小
 *
...
public class IntCudaPointer extends CudaPointer {
    ...
    public static IntCudaPointer malloc(int aCount) throws CudaException {
    ...
    private native static void fill_(long rPtr, int[] aData, int aStart, int aCount) throws CudaException;
     ...
     */
    @UnsafeJNI("Invalid input count may directly result in JVM SIGSEGV")
    public void fill(IntCPointer aData, int aCount) throws CudaException {
     ...
     */
    @UnsafeJNI("Invalid input count may directly result in JVM SIGSEGV")
    public void fill(IntCudaPointer aData, int aCount) throws CudaException {
        memcpy2this(aData, aCount*TYPE_SIZE);
    }
     ...
     */
    @UnsafeJNI("Invalid input count may directly result in JVM SIGSEGV")
    public void parse2dest(IntCPointer rDest, int aCount) throws CudaException {
     ...
     */
    @UnsafeJNI("Invalid input count may directly result in JVM SIGSEGV")
    public void parse2dest(IntCudaPointer rDest, int aCount) throws CudaException {
        memcpy2dest(rDest, aCount*TYPE_SIZE);
    }
