...
/**
 * 可以增长的 {@link FloatCudaPointer}，在增长时会自动释放旧的内存，并自动额外申请内存来加速；
 * 此外会记录当前的长度来判断是否需要增长
 * <p>
 * 仅仅进行内存管理，不会有任何初始值，并且扩容时不保留旧值
 * @author liqa
 */
public class GrowableFloatCudaPointer extends FloatCudaPointer {
    protected long mCount;
    @UnsafeJNI("Manual free required")
    public GrowableFloatCudaPointer(long aInitCount) throws CudaException {
        super(CudaCore.cudaMalloc(aInitCount*TYPE_SIZE));
        mCount = aInitCount;
    }
    public GrowableFloatCudaPointer() {
        super(0);
        mCount = 0;
    }
