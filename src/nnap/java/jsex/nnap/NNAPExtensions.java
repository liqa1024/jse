...
public class NNAPExtensions {
     ...
     */
    public static List<List<Vector>> calBasisPartialNNAP(final AtomicParameterCalculator self, final int aNMax, final int aLMax, final double aRCutOff, boolean aCalCross) {
        if (self.isShutdown()) throw new RuntimeException("This Calculator is dead");
        final int tThreadNum = self.threadNumber();
        IBasis[] tBasis = new IBasis[tThreadNum];
        for (int i = 0; i < tThreadNum; ++i) {
        ...
        try {
            ...
            // 理论上只需要遍历一半从而加速这个过程，但由于实现较麻烦且占用过多内存（所有近邻的 Ylm, Rn, fc 都要存，会随着截断半径增加爆炸增涨），这里不考虑
            self.pool_().parfor(self.atomNumber(), (i, threadID) -> {
                rOut.set(i, tBasis[threadID].evalPartial(aCalCross, self, i));
            });
    ...
    public static List<List<Vector>> calBasisPartialNNAP(final AtomicParameterCalculator self, final int aNMax, final int aLMax, final double aRCutOff) {return calBasisPartialNNAP(self, aNMax, aLMax, aRCutOff, false);}
}
